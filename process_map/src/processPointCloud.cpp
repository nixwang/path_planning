...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    if (New_Vertex)
    {
      ...
      cout << "\033[32m Do you want to keep this vertex [Y/N]?" << endl
           << " Or [S] for saving & processing PointCloud, [D] for deleting all vertices: "
           << "\033[0m";
      ...
      {
        cout << endl
             << "\033[32m Save the new vertex, and there are a total of " << polygonVertex.size() << " vertices."
             << "\033[0m" << endl;
        New_Vertex = false;
        break;
      }
      ...
      {
        cout << endl
             << "\033[32m Delete the specified point."
             << "\033[0m" << endl;
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_temp(new pcl::PointCloud<pcl::PointXYZ>);
        ...
        cout << endl
             << "\033[32m Save the DeleteOne PointCloud: " << oss.str() << "\033[0m" << endl;
        sensor_msgs::PointCloud2 pointCloud2msg;
        pcl::toROSMsg(*PointCloudLoad, pointCloud2msg);
        pointCloud2msg.header.frame_id = "odom";
        pointCloud2msg.header.stamp = ros::Time::now();
      ...
      case 'n':
      {
        ...
        cout << endl
             << "\033[32m Delete the new vertex, and there are " << polygonVertex.size() << " vertices left."
             << "\033[0m" << endl;
        New_Vertex = false;
        break;
      }
      ...
      case 's':
      {
        if (polygonVertex.size() >= 3)
        {
          ...
          pcl::io::savePCDFileASCII(oss.str(), *PointCloudLoad);
          cout << endl
               << "\033[32m Save the processed PointCloud: " << oss.str() << "\033[0m" << endl;
          sensor_msgs::PointCloud2 pointCloud2msg;
          pcl::toROSMsg(*PointCloudLoad, pointCloud2msg);
          pointCloud2msg.header.frame_id = "odom";
          pointCloud2msg.header.stamp = ros::Time::now();
        ...
        {
          cout << "\033[31m Less than 3 points cannot form a polygon. Pick again. "
               << "\033[0m" << endl;
        }
        polygonVertex.clear();
  ...
  string filtered_path = output_file + "car6.pcd";
